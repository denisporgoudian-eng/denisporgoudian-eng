
<h1 align="center">Je suis Denis PORGOUDIAN</h1>

<h3 align="center">
  Ingénieur d'État en Systèmes Embarqués & Informatique Industrielle
</h3>

<p align="center">
  <b>
    Embedded Systems • Firmware • Electronics & PCB • Industrial Automation
    <br>
    Automotive & ADAS • Embedded Communications • IoT • AI & Embedded Vision
  </b>
</p>

---

Je conçois et développe des **systèmes électroniques et embarqués** à l'intersection du **hardware, du firmware et des systèmes industriels**.

Mon approche couvre la **conception, le développement, l'intégration, la simulation, les tests et la validation** de solutions destinées notamment aux environnements **industriels, automobiles et IoT**.

### 🔧 Domaines

| | |
|---|---|
| 🧩 **Embedded Systems & Firmware** | Développement de systèmes embarqués et logiciels bas niveau |
| ⚡ **Electronics & PCB Design** | Conception électronique, schématique, PCB et validation hardware |
| 🔗 **Embedded Communications & IoT** | Interfaces, protocoles de communication et systèmes connectés |
| 🏭 **Industrial Automation & Energy** | Automatisation, contrôle-commande et systèmes industriels |
| 🚗 **Automotive & ADAS** | Systèmes automobiles, ADAS et développement orienté validation |
| 🤖 **AI & Embedded Vision** | Machine Learning, vision et déploiement sur plateformes embarquées |

---
## 👨‍💻 About Me

Je suis **Ingénieur d'État en Systèmes Embarqués & Informatique Industrielle**, avec une formation et des expériences orientées vers la conception de systèmes électroniques, le développement firmware et l'intégration de solutions industrielles et automobiles.

Mon profil couvre l'ensemble du cycle de développement d'un système embarqué :

**Conception → Développement → Intégration → Simulation → Tests → Validation**

### 🎯 Mon approche

Je m'intéresse particulièrement aux systèmes où le **hardware et le software doivent fonctionner ensemble de manière fiable et cohérente**. Mes projets m'ont amené à travailler aussi bien sur la conception de cartes électroniques et le développement de firmware que sur les communications embarquées, l'automatisation industrielle, le contrôle-commande et les applications automobiles.

J'accorde une attention particulière à :

- **Fiabilité & robustesse**
- **Performance & efficacité**
- **Maintenabilité & évolutivité**
- **Communication & intégration système**
- **Tests & validation**
- **Sécurité et contraintes des systèmes embarqués**

### 🌐 Domaines d'application

Mon parcours me permet d'aborder des problématiques dans différents environnements :

**🏭 Industrie** — automatisation, contrôle-commande, supervision et énergie  
**🚗 Automobile** — systèmes embarqués, ADAS, communications et validation  
**📡 IoT** — systèmes connectés, communications sans fil et acquisition de données  
**⚡ Électronique** — conception hardware, PCB, interfaces et intégration  
**🤖 IA embarquée** — vision, détection et déploiement sur plateformes embarquées

---
## ⚙️ Areas of Expertise

<table>
<tr>
<td width="50%" valign="top">

### 🧩 Embedded Systems & Firmware

Conception et développement de systèmes embarqués autour de microcontrôleurs et plateformes embarquées.

- Développement firmware en **C / C++**
- Architecture et programmation de microcontrôleurs
- Gestion des périphériques et interfaces matérielles
- Temps réel et bases des **RTOS**
- Intégration hardware / software
- Tests et validation de firmware

</td>
<td width="50%" valign="top">

### ⚡ Electronic Design & PCB

Conception et réalisation de cartes électroniques destinées aux systèmes embarqués et industriels.

- Conception de schémas électroniques
- Sélection et intégration des composants
- Conception et routage de **PCB**
- Simulation et vérification
- Génération des fichiers de fabrication
- Assemblage, mise au point et validation

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🔗 Embedded Communications & IoT

Mise en œuvre de communications entre systèmes embarqués et intégration de solutions connectées.

- Interfaces **UART, SPI, I²C**
- Communications **CAN / LIN**
- Communication sans fil et connectivité
- Échanges de données entre microcontrôleurs
- **Wi-Fi, Blutooth BLE, MQTT, LoRa**
- Architecture de systèmes IoT

</td>
<td width="50%" valign="top">

### 🏭 Industrial Automation & Energy

Conception et automatisation de systèmes industriels et analyse des problématiques énergétiques.

- Programmation des **PLC**
- Automatisation de procédés industriels
- Contrôle-commande
- **HMI / SCADA**
- Supervision et acquisition de données
- Analyse et optimisation de la consommation énergétique

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🚗 Automotive, ADAS & Functional Safety

Développement et étude de fonctions embarquées appliquées au domaine automobile.

- **ADAS**
- Model-Based Design
- Test **SIL/HIL**
- Communications automobiles
- Validation fonctionnelle
- Architecture et développement orientés automobile
- Introduction aux approches **AUTOSAR, ISO 26262, MISRA C & MAAB**

</td>
<td width="50%" valign="top">

### 🤖 AI & Embedded Vision

Application de techniques d'intelligence artificielle et de vision par ordinateur aux systèmes embarqués.

- Machine Learning
- Détection et reconnaissance d'objets
- **YOLO / R-CNN**
- Computer Vision
- Traitement d'images
- Déploiement sur plateformes embarquées

</td>
</tr>

<tr>
<td width="50%" valign="top">

### ⚙️ Control & Motorization

Étude et implémentation de systèmes de commande appliqués aux moteurs et aux systèmes dynamiques.

- Commande de moteurs **DC et pas à pas**
- Génération **PWM**
- Régulation **PI / PID**
- Acquisition et traitement de mesures
- Commande à base de microcontrôleurs
- Applications industrielles et embarquées

</td>
<td width="50%" valign="top">

### 🔬 System Integration & Validation

Intégration des différents éléments d'un système pour assurer son fonctionnement global.

- Intégration **Hardware / Firmware**
- Simulation et prototypage
- Tests fonctionnels
- Mise au point et débogage
- Validation des interfaces et communications
- Vérification du comportement système

</td>
</tr>
</table>

---
## 🛠️ Technologies & Tools

### 💻 Programming & Embedded Software

<p align="center">
  <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/C%2B%2B-00599C?style=for-the-badge&logo=cplusplus&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/MATLAB-FF8000?style=for-the-badge&logo=mathworks&logoColor=white" />
  <img src="https://img.shields.io/badge/Simulink-FF8000?style=for-the-badge&logo=mathworks&logoColor=white" />
  <img src="https://img.shields.io/badge/VHDL-543978?style=for-the-badge" />
</p>

### 🔌 Microcontrollers & Embedded Platforms

<p align="center">
  <img src="https://img.shields.io/badge/STM32-03234B?style=for-the-badge&logo=stmicroelectronics&logoColor=white" />
  <img src="https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge&logo=espressif&logoColor=white" />
  <img src="https://img.shields.io/badge/PIC16F877A-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Arduino-00979D?style=for-the-badge&logo=arduino&logoColor=white" />
  <img src="https://img.shields.io/badge/DSP_TMS320C2000-CC0000?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Embedded_Linux-000000?style=for-the-badge&logo=linux&logoColor=white" />
  <img src="https://img.shields.io/badge/FreeRTOS-000000?style=for-the-badge&logo=freertos&logoColor=white" />
</p>

### 📡 Embedded Communications & IoT

<p align="center">
  <img src="https://img.shields.io/badge/UART-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/SPI-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/I²C-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/CAN-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/LIN-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/BLE-0082FC?style=for-the-badge&logo=bluetooth&logoColor=white" />
  <img src="https://img.shields.io/badge/Wi--Fi-555555?style=for-the-badge&logo=wifi&logoColor=white" />
  <img src="https://img.shields.io/badge/MQTT-660066?style=for-the-badge&logo=mqtt&logoColor=white" />
  <img src="https://img.shields.io/badge/LoRa-1264A3?style=for-the-badge" />
</p>

### ⚡ Electronics & PCB Design

<p align="center">
  <img src="https://img.shields.io/badge/KiCad-314CB0?style=for-the-badge&logo=kicad&logoColor=white" />
  <img src="https://img.shields.io/badge/Altium_Designer-A5915F?style=for-the-badge&logo=altiumdesigner&logoColor=white" />
  <img src="https://img.shields.io/badge/EasyEDA-1A73E8?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Proteus-1F3C88?style=for-the-badge" />
  <img src="https://img.shields.io/badge/LTspice-900000?style=for-the-badge" />
  <img src="https://img.shields.io/badge/PSpice-900000?style=for-the-badge" />
  <img src="https://img.shields.io/badge/PSIM-555555?style=for-the-badge" />
</p>

### 🏭 Industrial Automation

<p align="center">
  <img src="https://img.shields.io/badge/Siemens_TIA_Portal-009999?style=for-the-badge&logo=siemens&logoColor=white" />
  <img src="https://img.shields.io/badge/STEP_7-009999?style=for-the-badge&logo=siemens&logoColor=white" />
  <img src="https://img.shields.io/badge/Schneider_Unity_Pro-00874E?style=for-the-badge" />
  <img src="https://img.shields.io/badge/EcoStruxure-00874E?style=for-the-badge" />
  <img src="https://img.shields.io/badge/PLC-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/HMI-555555?style=for-the-badge" />
  <img src="https://img.shields.io/badge/SCADA-555555?style=for-the-badge" />
</p>

### 🚗 Automotive & Model-Based Design

<p align="center">
  <img src="https://img.shields.io/badge/CANoe-007ACC?style=for-the-badge" />
  <img src="https://img.shields.io/badge/CANalyzer-007ACC?style=for-the-badge" />
  <img src="https://img.shields.io/badge/AUTOSAR-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/ISO_26262-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/MISRA_C-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/MAAB-2C2C2C?style=for-the-badge" />
</p>

### 🤖 AI, Computer Vision & Embedded AI

<p align="center">
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/YOLO-111111?style=for-the-badge" />
  <img src="https://img.shields.io/badge/R--CNN-111111?style=for-the-badge" />
  <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white" />
  <img src="https://img.shields.io/badge/NVIDIA_Jetson-76B900?style=for-the-badge&logo=nvidia&logoColor=white" />
</p>

### 🔧 Engineering & Development Tools

<p align="center">
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white" />
  <img src="https://img.shields.io/badge/Keil-555555?style=for-the-badge" />
</p>

---
## 🚀 Featured Projects

> Une sélection de projets représentatifs de mon parcours en **systèmes embarqués, électronique, automatisation industrielle, automobile, contrôle et intelligence artificielle**.

---

### 🔌 PFE — Modular Embedded & IoT Board

**Conception et réalisation d'une carte électronique embarquée dédiée aux expérimentations autour des microcontrôleurs et de l'IoT.**

Le projet couvre l'ensemble de la chaîne de développement, de la conception électronique jusqu'à l'intégration du firmware et des communications.

**🔧 Réalisation**
- Architecture et conception du système électronique
- Conception du schéma et du **PCB**
- Simulation et prototypage
- Assemblage et validation de la carte
- Développement du firmware sur **PIC16F877A**
- Communication **PIC ↔ ESP32**
- Interfaces **UART, GPIO, ADC**
- Intégration de capteurs et périphériques
- Connectivité **Wi-Fi & MQTT**

**🛠️ Technologies**
`PIC16F877A` `ESP32` `C` `UART` `ADC` `LCD` `Keypad` `PCB` `EasyEDA` `Proteus` `MQTT`

🔗 **[View Project →](https://github.com/denisporgoudian-eng/pfe-modular-embedded-iot-board)**

---

### 🚗 ADAS — Traffic Sign Detection & Recognition

**Développement d'une solution de détection et de reconnaissance de panneaux routiers destinée à une application orientée ADAS.**

**🔧 Réalisation**
- Préparation et traitement des données
- Développement d'une solution de détection d'objets
- Entraînement et évaluation des modèles
- Détection et reconnaissance des panneaux routiers
- Expérimentation sur plateforme embarquée
- Analyse des performances du modèle

**🛠️ Technologies**
`Python` `PyTorch` `YOLO` `R-CNN` `Computer Vision` `NVIDIA Jetson Nano`

**🎯 Domaine**
`Automotive` `ADAS` `Embedded AI` `Computer Vision`

---

### 🏭 Automatisation du lavage des filtres

**Automatisation d'un processus industriel de lavage de filtres avec une approche orientée contrôle-commande.**

**🔧 Réalisation**
- Analyse du processus industriel
- Définition de la séquence de fonctionnement
- Programmation de l'automate
- Gestion des actionneurs et conditions de fonctionnement
- Développement de la logique de commande
- Simulation et validation du fonctionnement

**🛠️ Technologies**
`Schneider Unity Pro` `PLC` `Automation` `Control Systems`

---

### ⚡ Analyse & Automatisation de la Consommation Énergétique

**Étude et mise en œuvre d'une solution d'automatisation appliquée à l'analyse de la consommation énergétique industrielle.**

**🔧 Réalisation**
- Analyse du fonctionnement du système
- Acquisition et exploitation des données
- Analyse de la consommation énergétique
- Mise en œuvre de la logique d'automatisation
- Supervision et suivi des paramètres
- Analyse des possibilités d'amélioration de l'efficacité énergétique

**🛠️ Technologies**
`Siemens TIA Portal` `STEP 7` `PLC` `HMI` `Energy Management`

---

### ⚙️ Motor Control & PI/PID Regulation

**Étude et implémentation de systèmes de commande et de régulation de moteurs à base de systèmes embarqués.**

**🔧 Réalisation**
- Commande de moteurs **DC**
- Commande de moteurs **pas à pas**
- Génération de signaux **PWM**
- Acquisition de mesures
- Mise en œuvre de régulateurs **PI / PID**
- Expérimentation sur microcontrôleur
- Étude du comportement dynamique du système

**🛠️ Technologies**
`PIC16F877A` `DSP TMS320C2000` `PWM` `PI` `PID` `C`

---

### 🚘 Automotive Model-Based Design — ACC

**Modélisation et étude d'une fonction Adaptive Cruise Control dans un environnement automobile.**

**🔧 Réalisation**
- Modélisation du comportement du système
- Définition de la logique de contrôle
- Simulation de la fonction **ACC**
- Analyse du comportement dans différents scénarios
- Validation fonctionnelle du modèle

**🛠️ Technologies**
`MATLAB` `Simulink` `Model-Based Design` `ACC`

---

### 💡 Automotive Exterior Lighting Control

**Étude et modélisation de la logique de commande des éclairages extérieurs d'un véhicule.**

**🔧 Réalisation**
- Analyse des exigences fonctionnelles
- Modélisation de la logique de commande
- Gestion des différents états du système
- Simulation
- Vérification du comportement fonctionnel

**🛠️ Technologies**
`MATLAB` `Simulink` `Model-Based Design`

---

### 🔋 DC-DC Buck Converter

**Conception et étude d'un convertisseur DC-DC de type Buck pour une application d'électronique de puissance.**

**🔧 Réalisation**
- Analyse du principe de fonctionnement
- Dimensionnement des composants
- Étude de la conversion d'énergie
- Simulation du circuit
- Analyse des paramètres électriques
- Évaluation du comportement du convertisseur

**🛠️ Technologies**
`Power Electronics` `DC-DC` `PWM` `PSIM` `Simulation`

---

### 🤖 Embedded & Robotics Projects

**Ensemble de projets pratiques développés autour des microcontrôleurs, de la robotique et de l'intégration de systèmes.**

**Projets réalisés**
- Line Follower
- Bras robotisé
- Système de détection d'incendie
- Acquisition et traitement de données
- Intégration de capteurs et actionneurs

**🛠️ Technologies**
`Arduino` `Embedded Systems` `Sensors` `Actuators` `Robotics`

---
## 🏭 Industrial Experience

### 💧 ONEP — Office National de l'Eau Potable

**Stage d'initiation — 2024**

Découverte de l'environnement industriel et des systèmes techniques liés aux installations de production et de distribution d'eau.

**Principales activités :**

- Observation et compréhension des installations industrielles
- Découverte des équipements électriques et d'instrumentation
- Observation des systèmes de contrôle et d'automatisation
- Analyse du fonctionnement des processus industriels
- Familiarisation avec les contraintes de maintenance et d'exploitation

---

### 🏗️ Edifice Prefa

**Stage de fin d'année — 2026**

Participation aux activités techniques liées à l'automatisation et au fonctionnement des installations industrielles.

**Principales activités :**

- Analyse de processus industriels
- Participation à la programmation et à l'automatisation
- Étude de systèmes de contrôle-commande
- Analyse de la consommation énergétique
- Participation aux travaux d'amélioration des systèmes
- Observation et application des pratiques de maintenance industrielle

---

### 🏭 Industrial Automation & Control

Au cours de mon parcours, j'ai également travaillé sur plusieurs problématiques liées à l'**automatisation industrielle**, notamment :

- Programmation de **PLC Siemens et Schneider**
- Automatisation de processus industriels
- Développement de séquences de commande
- Supervision et interfaces **HMI / SCADA**
- Analyse de systèmes électromécaniques
- Acquisition et traitement de données
- Étude de la consommation énergétique
- Mise en œuvre de solutions de contrôle-commande

> **Objectif professionnel :** mettre mes compétences en systèmes embarqués, électronique, automatisation et développement logiciel au service de systèmes industriels et technologiques fiables et performants.

---
## 🎓 Education

### 🎓 Ingénieur d'État — Systèmes Embarqués & Informatique Industrielle

**2023 – 2026**

Formation d'ingénieur orientée vers la conception et l'intégration de systèmes électroniques, embarqués et industriels.

**Principaux domaines étudiés :**

- Systèmes embarqués & microcontrôleurs
- Développement firmware
- Électronique analogique & numérique
- Conception de systèmes électroniques & PCB
- Automatique & contrôle-commande
- Automatisation industrielle
- Réseaux & communications industrielles
- Systèmes automobiles & ADAS
- MATLAB / Simulink & Model-Based Design
- Intelligence artificielle & vision par ordinateur
- IoT & systèmes connectés

---

### 📘 DEUST — Mathématiques, Informatique & Physique

**2021 – 2023**

Formation scientifique pluridisciplinaire constituant la base de mon parcours en ingénierie.

**Domaines principaux :**

- Mathématiques
- Informatique
- Physique
- Électronique
- Algorithmique
- Bases des systèmes et sciences de l'ingénieur

---
## 🤝 Leadership & Activities

### 🇧🇫 Association des Étudiants et Stagiaires Burkinabè au Maroc — AEBM

**Président — Beni Mellal**

Responsabilité associative impliquant la coordination des membres, l'organisation des activités et la représentation de l'association.

- Coordination et animation d'équipe
- Organisation d'activités et d'événements
- Gestion de projets associatifs
- Communication et représentation
- Coordination avec différents interlocuteurs
- Prise d'initiative et résolution de problèmes

---

### 🎭 AEBM — Affaires Socio-Culturelles

**Chargé aux affaires socio-culturelles**

Participation à l'organisation et à la coordination d'activités sociales, culturelles et collectives au sein de la communauté estudiantine.

---

### 🔬 EMBATRIC

**Membre actif**

Participation à des projets et activités techniques autour de l'**électronique, des systèmes embarqués, de la robotique et du prototypage**.

---

### 🌍 CESAM

**Membre actif**

Participation aux activités et initiatives de l'association, avec une implication dans le travail collectif et les projets étudiants.

---

### 💡 Leadership & Professional Skills

<p align="center">
  <img src="https://img.shields.io/badge/Leadership-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Project_Management-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Teamwork-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Communication-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Problem_Solving-2C2C2C?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Adaptability-2C2C2C?style=for-the-badge" />
</p>

---
## 📊 GitHub

<p align="center">
  <a href="https://github.com/denisporgoudian-eng">
    <img src="https://img.shields.io/badge/GitHub-denisporgoudian--eng-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=denisporgoudian-eng&show_icons=true&theme=github_dark&hide_border=true&rank_icon=github&include_all_commits=true" height="170" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=denisporgoudian-eng&layout=compact&theme=github_dark&hide_border=true&langs_count=8" height="170" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=denisporgoudian-eng&theme=github-dark-blue&hide_border=true" />
</p>

### 🔧 Engineering Focus

<p align="center">
  <img src="https://img.shields.io/badge/Embedded_Systems-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/Firmware-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/Electronics-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/PCB_Design-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/Industrial_Automation-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/Automotive-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/IoT-0D1117?style=flat-square" />
  <img src="https://img.shields.io/badge/Embedded_AI-0D1117?style=flat-square" />
</p>

<p align="center">
  <i>Building reliable systems at the intersection of hardware, firmware and industrial engineering.</i>
</p>

---
## 📫 Contact

<p align="center">
  <a href="mailto:denisporgoudian@gmail.com">
    <img src="https://img.shields.io/badge/Email-denisporgoudian%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="[https://www.linkedin.com/](https://www.linkedin.com/in/ingenieur-automobile-hardware-designer-syst%C3%A8mes-embarqu%C3%A9-iot-automatismes/)">
    <img src="https://img.shields.io/badge/LinkedIn-Denis_Porgoudian-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/denisporgoudian-eng">
    <img src="https://img.shields.io/badge/GitHub-denisporgoudian--eng-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

<p align="center">
  <b>Ingénieur en Systèmes Embarqués & Informatique Industrielle</b>
  <br>
  Embedded Systems • Electronics • Firmware • Industrial Automation • Automotive • IoT
</p>

---

<p align="center">
  <i>Concevoir. Développer. Intégrer. Valider.</i>
</p>
